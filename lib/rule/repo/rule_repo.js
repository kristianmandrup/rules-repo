// Generated by LiveScript 1.2.0
(function(){
  var Debugger, RuleContainer, RuleRepo;
  Debugger = require('../../util').Debugger;
  RuleContainer = require('../container').RuleContainer;
  module.exports = RuleRepo = (function(){
    RuleRepo.displayName = 'RuleRepo';
    var prototype = RuleRepo.prototype, constructor = RuleRepo;
    importAll$(prototype, arguments[0]);
    function RuleRepo(name, options, debugging){
      this.name = name;
      this.options = options != null
        ? options
        : {};
      this.debugging = debugging != null ? debugging : true;
    }
    prototype._type = 'RuleRepo';
    prototype.container = function(){
      return this._container || (this._container = this.options.container || this.defaultContainer());
    };
    prototype.defaultContainer = function(){
      return new RuleContainer(this.debugging);
    };
    prototype.canRules = function(){
      return this.container().can;
    };
    prototype.cannotRules = function(){
      return this.container().cannot;
    };
    prototype.display = function(){
      return this.container().display;
    };
    prototype.register = function(act, actions, subjects){
      return this.container().register.apply(this.container(), arguments);
    };
    prototype.match = function(act, accessRequest){
      return this.container().match.apply(this.container(), arguments);
    };
    prototype.clean = function(){
      this.container().clean();
      return this;
    };
    return RuleRepo;
  }(Debugger));
  function importAll$(obj, src){
    for (var key in src) obj[key] = src[key];
    return obj;
  }
}).call(this);
